...
public class PullToRefreshSwipsListView extends PullToRefreshAdapterViewBase<SwipeListView> {

	private LoadingLayout mHeaderLoadingView;
	private LoadingLayout mFooterLoadingView;
	private FrameLayout mLvFooterLoadingFrame;

	public PullToRefreshSwipsListView(Context context) {
		super(context);
		setDisableScrollingWhileRefreshing(false);
	}

	public PullToRefreshSwipsListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		setDisableScrollingWhileRefreshing(true);
	}

	public PullToRefreshSwipsListView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		setDisableScrollingWhileRefreshing(true);
	}
	
	public PullToRefreshSwipsListView(Context context, Mode mode) {
		super(context, mode);
		setDisableScrollingWhileRefreshing(true);
	}
	...
	void onPullToRefresh() {
		...
		mRefreshableView.getTouchListener().closeOpenedItems(true);
//		mRefreshableView.getTouchListener().setEnabled(false);
	}
	
	@Override
	public void onRefreshComplete() {
		super.onRefreshComplete();
		mRefreshableView.getTouchListener().setEnabled(true);
	}
	...
	public void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount,
		...
		mRefreshableView.getScrollListener().onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
	}
	...
	public void onScrollStateChanged(final AbsListView view, final int scrollState) {
		...
		mRefreshableView.getScrollListener().onScrollStateChanged(view, scrollState);
	}
	...
	protected final SwipeListView createRefreshableView(Context context, AttributeSet attrs) {
		SwipeListView swipe_lv = createListView(context, attrs);

		// Get Styles from attrs
		TypedArray attr_pulltorefresh = context.obtainStyledAttributes(attrs, R.styleable.PullToRefresh);
		
		// Create Loading Views ready for use later
		FrameLayout frame = new FrameLayout(context);
		mHeaderLoadingView = createLoadingLayout(context, Mode.PULL_DOWN_TO_REFRESH, attr_pulltorefresh);
		frame.addView(mHeaderLoadingView, FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.WRAP_CONTENT);
		mHeaderLoadingView.setVisibility(View.GONE);
		swipe_lv.addHeaderView(frame, null, false);

		mLvFooterLoadingFrame = new FrameLayout(context);
		mFooterLoadingView = createLoadingLayout(context, Mode.PULL_UP_TO_REFRESH, attr_pulltorefresh);
		mLvFooterLoadingFrame.addView(mFooterLoadingView, FrameLayout.LayoutParams.MATCH_PARENT,
				FrameLayout.LayoutParams.WRAP_CONTENT);
		mFooterLoadingView.setVisibility(View.GONE);

		attr_pulltorefresh.recycle();

		// Set it to this so it can be used in ListActivity/ListFragment
		swipe_lv.setId(android.R.id.list);
		return swipe_lv;
	}
	...
	final class InternalListViewSDK9 extends InternalListView {
		...
		protected boolean overScrollBy(int deltaX, int deltaY, int scrollX, int scrollY, int scrollRangeX,
			...
			OverscrollHelper.overScrollBy(PullToRefreshSwipsListView.this, deltaY, scrollY, isTouchEvent);
	...
	protected class InternalListView extends SwipeListView implements EmptyViewMethodAccessor {
		...
		public void setEmptyView(View emptyView) {
			PullToRefreshSwipsListView.this.setEmptyView(emptyView);
		}
